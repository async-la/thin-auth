...
export { CREDENTIAL_TYPE_DEV, CREDENTIAL_TYPE_EMAIL, CREDENTIAL_TYPE_SMS } from '@rt2zz/thin-auth-interface'
  ...
  if (sign) {
    ...
    authRemote.sign(
      async nonce => {
        ...
        let [api, keypair] = await Promise.all([await cryptoRemote(), keypairAtom && keypairAtom.get()])
