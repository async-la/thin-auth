...
async function enforceActiveSession(Session: any, sessionId: string): Promise<SessionType> {
...
async function enforceLatentSession(Session: any, sessionId: string): Promise<SessionType> {
  let session = await Session.findOne({ where: { id: sessionId } })
  let now = new Date()
...
async function sendLoginLink(
  ...
  let cipherPayload: CipherPayload = [sessionId, operation, [createAlias.credential, createAlias.type], undefined]
...
async function approveAuth(cipher: string): Promise<void> {
  ...
  if (session.verifiedAt === null)
    operations[0] = Session.update({ verifiedAt: new Date() }, { where: { id: sessionId } })

  let [sessionResult, newAliasResult, oldAliasResult] = await Promise.all(operations)
  if (session.verifiedAt === null) {
